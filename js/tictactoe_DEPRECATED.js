$(document).ready(function () {
    ...
    $smallBox.on('click', function (event) {
        ...
        if (!$this.hasClass('played') && winner === undefined) {
            ...
            if (aiPlay && winner === undefined) {
                const bestBox = sendInfoToAI();
                updateAIMove(bestBox);
                winnerReason = checkWin('square', `${ bestBox }`);
                winner = winnerReason[0];
                reason = winnerReason[1];
                nextPlayer = playerOne;
    ...
    function sendInfoToAI () {
        const posAI = playerPositions.square;
        const posOppo = playerPositions.naught;
        const bestBox = askAI(posAI, posOppo);
        return bestBox;
    };

    function updateAIMove (bestBox) {
        const $aiPicks = $(`#${ bestBox }`);
        $aiPicks.addClass('played');
        $('<div></div>').hide().addClass('square').appendTo($aiPicks).fadeIn(200);
    };

    const emptyBoxes = [[0, 0, 0], [0, 0, 0], [0, 0, 0]];

    function askAI (posAI, posOppo) {
        const emptyBoxes = findEmptyBoxes(posAI, posOppo);
        const bestBox = findBestBox(posAI, posOppo, emptyBoxes);
        return bestBox;
    };

    function findEmptyBoxes (posAI, posOppo) {
        for (let i = 0; i < emptyBoxes.length; i++) {
            for (let j = 0; j < emptyBoxes.length; j++) {
                emptyBoxes[i][j] = posAI[i][j] ^ posOppo[i][j];
            }
        }
        return emptyBoxes;
    };

    function findBestBox (posAI, posOppo, emptyBoxes) {
        let depth = 0;
